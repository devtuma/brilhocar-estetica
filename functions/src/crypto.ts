...
function getMasterKey(): Buffer {
  ...
  if (!masterKey) {
    throw new Error(
      '[crypto] MASTER_KEY não configurada. Execute: firebase functions:config:set crypto.master_key="..."'
    );
  }
 ...
 */
export function decrypt(ciphertextBase64: string, tenantId: string): string {
 ...
 */
export function decryptIfEncrypted(value: string, tenantId: string): string {
  if (!value) return '';
